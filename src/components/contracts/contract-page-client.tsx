...
import {
  ...
  readEmployerContractRecords,
  saveEmployerContractRecordToCloud,
...
export function ContractPageClient({ embedded = false }: { embedded?: boolean }) {
  ...
  const [contractRecords, setContractRecords] = useState<EmployerContractRecord[]>([]);
  ...
  useEffect(() => {
    ...
    const nextRecords = readEmployerContractRecords();
    setActiveCompany(nextActiveCompany);
    setContractRecords(nextRecords);
    if (!nextActiveCompany) return;

    let cancelled = false;
    fetchEmployerContractRecordsFromCloud(nextActiveCompany.id)
      .then((cloudRecords) => {
        if (cancelled || !cloudRecords) return;
        setContractRecords(cloudRecords);
        writeEmployerContractRecords(cloudRecords);
      })
      .catch(() => {
        if (!cancelled) setArchiveMessage("Contrats cloud indisponibles, archive locale conservee.");
      });
  ...
  function buildContractFilename(contract: GeneratedContract) {
    return `contrat_simpaie_${contract.id.slice(0, 8)}.txt`;
  }

  function createEmployerContractRecord(formData: ContractFormData, contract: GeneratedContract): EmployerContractRecord {
    const employees = readEmployerEmployees() ?? [];
    const employee = employees.find(
      (item) => item.fullName.trim().toLowerCase() === formData.employee_name.trim().toLowerCase(),
    );
    ...
    return {
      ...
      employeeName: formData.employee_name || employee?.fullName || "A completer",
      contractType: formData.contract_type,
      contractDate: formData.contract_date || new Date().toISOString().slice(0, 10),
      status: "generated",
      filename: buildContractFilename(contract),
      content: contract.content,
      contractData: (contract.contractData ?? formData) as unknown as Record<string, unknown>,
      warnings: contract.warnings,
      createdAt: contract.createdAt ?? new Date().toISOString(),
    };
  }

  function replaceContractRecordInList(records: EmployerContractRecord[], record: EmployerContractRecord) {
    return records.some((item) => item.id === record.id)
      ? records.map((item) => (item.id === record.id ? record : item))
      : [record, ...records];
  }

  async function persistContractRecord(record: EmployerContractRecord, successMessage: string) {
    if (!activeCompany) return;
    try {
      const savedRecord = await saveEmployerContractRecordToCloud(activeCompany.id, record);
      if (!savedRecord) throw new Error("unauthorized");
      setContractRecords((current) => {
        const nextRecords = replaceContractRecordInList(current, savedRecord).slice(0, 100);
        writeEmployerContractRecords(nextRecords);
        return nextRecords;
      });
      setArchiveMessage(successMessage);
    } catch {
      setArchiveMessage("Contrat genere, mais archive cloud non sauvegardee.");
    }
  }
  ...
  const handleGenerate = async (formData: ContractFormData) => {
    ...
    try {
      ...
      const contract = data.contract as GeneratedContract;
      setGeneratedContract(contract);
      if (embedded && activeCompany) {
        const nextRecord = createEmployerContractRecord(formData, contract);
        void persistContractRecord(nextRecord, `Contrat archive pour ${nextRecord.employeeName}.`);
      }
    } catch (err) {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    const fileName = buildContractFilename(generatedContract);
    a.download = fileName;
    ...
    if (embedded && activeCompany) {
      const record = contractRecords.find(
        (item) => item.generatedContractId === generatedContract.id || item.id === generatedContract.id,
      );
      if (record) {
        void persistContractRecord({ ...record, status: "downloaded", filename: fileName }, "Contrat marque telecharge.");
      }
    }
  };
  ...
  const renderPage = (
    content: ReactNode,
    className = "paper-bg min-h-screen max-w-full overflow-x-hidden",
    withDir = false,
  ) => {
    const dir = withDir ? (language === "ar" ? "rtl" : "ltr") : undefined;
    ...
    return (
      <main className={className} dir={dir}>
        {content}
      </main>
    );
  };
    ...
    return renderPage(
      <>
        <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6">
          <div className={`${embedded ? "rounded-lg border border-[var(--line)] bg-[var(--surface)]" : "soft-card rounded-[2rem]"} p-8 text-center`}>
  ...
  if (error) {
    return renderPage(
      <>
        <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6">
          <div className={`${embedded ? "rounded-lg border border-[var(--line)] bg-[var(--surface)]" : "soft-card rounded-[2rem]"} space-y-4 p-8 text-center`}>
  ...
  if (generatedContract) {
    return renderPage(
      <>
        <div className={`relative z-10 w-full ${embedded ? "" : "mx-auto max-w-5xl px-4 py-8 sm:px-6"}`}>
          {/* Success Header */}
          <section className={`${embedded ? "rounded-lg border border-[var(--line)] bg-[var(--surface)]" : "soft-card rounded-[2rem]"} mb-6 p-6 sm:p-8`}>
            ...
            {embedded && archiveMessage ? (
              <p className="mt-4 rounded-lg bg-[var(--surface-muted)] px-3 py-2 text-sm text-[var(--ink-soft)]">
                {archiveMessage}
              </p>
            ) : null}
          </section>
      ...
      "paper-bg min-h-screen max-w-full overflow-x-hidden pb-24",
    );
  }
  ...
  return renderPage(
    <>
      <div className={`relative z-10 w-full ${embedded ? "pb-0" : "mx-auto max-w-6xl px-4 pb-8 pt-24 sm:px-6"}`}>
        ...
        {embedded ? (
          <section className="mb-4 rounded-lg border border-[var(--line)] bg-[var(--surface)] p-5">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--accent)]">
                  Archive
                </p>
                <h2 className="mt-2 text-xl font-black">Contrats generes</h2>
              </div>
              <span className="rounded-full bg-[var(--surface-muted)] px-3 py-1 text-xs font-bold text-[var(--ink-soft)]">
                {contractRecords.length} contrat(s)
              </span>
            </div>
            {archiveMessage ? (
              <p className="mt-3 rounded-lg bg-[var(--surface-muted)] px-3 py-2 text-sm text-[var(--ink-soft)]">
                {archiveMessage}
              </p>
            ) : null}
            <div className="mt-4 grid gap-3 lg:grid-cols-2">
              {contractRecords.length === 0 ? (
                <p className="text-sm text-[var(--ink-soft)]">
                  Aucun contrat archive pour le contexte actif.
                </p>
              ) : (
                contractRecords.slice(0, 4).map((record) => (
                  <button
                    key={record.id}
                    type="button"
                    onClick={() => {
                      setGeneratedContract({
                        id: record.generatedContractId ?? record.id,
                        content: record.content,
                        templateId: record.contractType,
                        contractData: record.contractData as unknown as ContractFormData,
                        warnings: record.warnings,
                        createdAt: record.createdAt,
                      });
                    }}
                    className="rounded-lg border border-[var(--line)] bg-[var(--surface-muted)] p-3 text-left transition hover:border-[var(--accent)]"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="truncate text-sm font-black text-[var(--heading)]">{record.employeeName}</p>
                        <p className="mt-1 text-xs text-[var(--ink-soft)]">
                          {record.contractType} - {new Date(record.createdAt).toLocaleString("fr-MA")}
                        </p>
                      </div>
                      <span className="shrink-0 rounded-full bg-[var(--surface)] px-2.5 py-1 text-[11px] font-bold text-[var(--accent)]">
                        {record.status === "downloaded" ? "Telecharge" : "Genere"}
                      </span>
                    </div>
                  </button>
                ))
        ...
        {/* Wizard */}
        <div className={`${embedded ? "rounded-lg border border-[var(--line)] bg-[var(--surface)]" : "soft-card rounded-[2rem]"} p-4 sm:p-8`}>
          <ContractWizard
            templates={templates}
            clauses={clauses}
